...
# Variáveis Globais
ubd = 'user_migracao'   # Usurário Para Acesso ao Banco
pbd = 'Mig@Arquivar$17' # Senha do Usuário
novo_id_extensao = 1
...
def verificar_arquivo(codigo,nome_arquivo):
    ...
    conn = pyodbc.connect(
        'Driver={SQL Server};'
        'Server=arqged.database.windows.net;'  # Nome do servidor SQL Server
        'Database=arquivar_producao;'          # Nome do banco de dados
...
if tp == 0:
    ...
    ccc = input('\nINSIRA A CADEIA DE CARACTERES DE CONSULTA: ')
        ...
        for linha in tqdm(arquivo, total=total_linhas, desc='Progresso'):
            ...
            if existe_arquivo:
                with open (ptl + '\\arquivo_ja_existentes.txt', 'a') as arquivo:
                    arquivo.write(f'O documento "{codigo}", já possui um documento com o nome "{nome_arquivo}" associado.\n')
                ...
                if idextensao is not None:
                    ...
                    if num_paginas is not None:
                        with open(pt + '\\01_ComandoSQL.sql', 'a') as arquivo:
                            arquivo.write(f"INSERT INTO GedDocumento.Imagem VALUES ('{id.upper()}','{iddocumento.upper()}',1,'http://{uconta}.blob.core.windows.net/{cid.lower()}/{id.lower()}.{extensao.lower()}','{nome_arquivo}',{num_paginas},{tamanho},GETDATE(),{idextensao[0]},'{user_resp.upper()}',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);\n")
                        
                        with open(pt + '\\01_ComandoPS.bat', 'a') as arquivo:
                            arquivo.write(f".\\azcopy cp '{caminho}' 'https://{uconta}.blob.core.windows.net/{cid.lower()}/{id.lower()}.{extensao.lower()}{ccc}' --recursive=true\n")
                            
                        with open(ptl + '\\logSucesso.txt', 'a') as arquivo:
                           arquivo.write(f"DE: {nome_arquivo} | PARA: {id.upper()}\n")
                    else:
                        with open(pt + '\\01_ComandoSQL.sql', 'a') as arquivo:
                            arquivo.write(f"INSERT INTO GedDocumento.Imagem VALUES ('{id.upper()}','{iddocumento.upper()}',1,'http://{uconta}.blob.core.windows.net/{cid.lower()}/{id.lower()}.{extensao.lower()}','{nome_arquivo}',NULL,{tamanho},GETDATE(),{idextensao[0]},'{user_resp.upper()}',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);\n")
                    ...
                    id_extensao = int(verifica_id_ext()) + novo_id_extensao
                    
                    with open(pt + '\\00_ComandoSQL.sql', 'a') as arquivo:
                        arquivo.write(f"INSERT INTO GedSistema.Extensao VALUES ({id_extensao},'{extensao.upper()}','<i class=""fa fa-file"" aria-hidden=""true"" style=""font-size:18px;color: gray;""></i>');\n")
                        
                    novo_id_extensao += 1
                    
                    with open(ptl + '\\logSucessoExtensao.txt', 'a') as arquivo:
                        arquivo.write(f"Foi gerado o insert para a extensão {extensao.upper()}.\n")
                        
                    with open(pt + '\\01_ComandoSQL.sql', 'a') as arquivo:
                        arquivo.write(f"INSERT INTO GedDocumento.Imagem VALUES ('{id.upper()}','{iddocumento.upper()}',1,'http://{uconta}.blob.core.windows.net/{cid.lower()}/{id.lower()}.{extensao.lower()}','{nome_arquivo}',NULL,{id_extensao},'{user_resp.upper()}',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);\n")
                        
                    with open(ptl + '\\logSucesso', 'a') as arquivo:
                        arquivo.write(f"DE: {nome_arquivo} | PARA: {id.upper()}\n")
    ...
    account_name = "arquivarstoragetmp" # ucontao
    account_key = "sJiLm6eSiIxO3l5dOn94F2I+UOdxhumzxaly7SA68LuyxjArp2aBg0HiVwWRWn4AkWzlIAyiUkXCJCU6kb1vOA==" # usenhao
    source_container_name = "seac-08-18" # cido.lower()
    ccco = input('\nINSIRA A CADEIA DE CARACTERES DE CONSULTA DE ORIGEM: ')
    ...
    destination_account_name = '15833623000112'
    destination_account_key = 'ixxlBn/0YcVlPaMVz5KzIynmKaFU0+pYPPMlPVv8Pbq/1VAqkfwlh5uq/tz5Cw+XnXm5yrejgZwvlgau9cUpoQ=='
    destination_container_name = '115e5680-7d1b-43b4-a7f6-577324f6db36'
    
    print('\n')
        
    with open(r"D:\Cebolinha\OneDrive - Arquivar Franchising Ltda\General\TASK\Felipe\PYTHON\ASSOCIAR IMAGENS A REGISTROS ARQGED SEAC\\arquivos.txt", 'r') as arquivo: 
        ...
        for linha in tqdm(arquivo, total=total_linhas, desc='Progresso'):
            ...
            # codigo, idimagem = linha.strip().split('|')
            codigo, caminho, arquivo_nome = linha.strip().split('|')
            ...
            destination_blob_name = f"{id_img}.{extensao.lower()}"
            ...
            #id_documento = buscar_documento(codigo)
            ...
            container_name = "seac-08-18"
            blob_name = caminho
            
            if not os.path.exists(r"D:\Cebolinha\OneDrive - Arquivar Franchising Ltda\General\TASK\Felipe\PYTHON\ASSOCIAR IMAGENS A REGISTROS ARQGED SEAC\arquivos"):
                os.makedirs(r"D:\Cebolinha\OneDrive - Arquivar Franchising Ltda\General\TASK\Felipe\PYTHON\ASSOCIAR IMAGENS A REGISTROS ARQGED SEAC\arquivos")
                
            arquivo_extensao = f'{arquivo_nome}.{extensao.lower()}'
            caminho_arquivo = os.path.join(r"D:\Cebolinha\OneDrive - Arquivar Franchising Ltda\General\TASK\Felipe\PYTHON\ASSOCIAR IMAGENS A REGISTROS ARQGED SEAC\arquivos", arquivo_extensao)
            ...
            tamanho = os.path.getsize(f'{caminho_arquivo}')    

            try:
                qnt_paginas = contar_paginas_pdf(f'{caminho_arquivo}')
            except Exception as errofitz:
                with open (r'D:\Cebolinha\OneDrive - Arquivar Franchising Ltda\General\TASK\Felipe\PYTHON\ASSOCIAR IMAGENS A REGISTROS ARQGED SEAC\\LogErro.txt', 'a') as arquivoFitz:
                    arquivoFitz.write(f"ErroFitz: {errofitz} | Blob: {caminho_arquivo}\n")
            ...
            if blob_client_destination.exists():
                with open ('\\LogErroExistente.txt', 'a') as arquivo:
                    arquivo.write(f"Arquivo '{caminho}' já existente no destino | IdImagem: '{id_img.upper()}'\n")
                ...
                try:
                    ...
                    with open (r'D:\Cebolinha\OneDrive - Arquivar Franchising Ltda\General\TASK\Felipe\PYTHON\ASSOCIAR IMAGENS A REGISTROS ARQGED SEAC\\LogSucesso.txt', 'a') as arquivo:
                        arquivo.write(f"DE: {caminho} | PARA: {id_img.lower()}.{extensao.lower()}\n")
                        os.remove(caminho_arquivo) 

                    with open (r'D:\Cebolinha\OneDrive - Arquivar Franchising Ltda\General\TASK\Felipe\PYTHON\ASSOCIAR IMAGENS A REGISTROS ARQGED SEAC\\insert.sql', 'a', encoding= "utf-8") as insert:
                        insert.write(f"INSERT INTO Geddocumento.imagem VALUES('{id_img.lower()}','{iddocumento.lower()}','1','https://{destination_account_name}.blob.core.windows.net/{destination_container_name}/{id_img.lower()}.{extensao.lower()}','{arquivo_nome}','{qnt_paginas}','{tamanho}',GETDATE(),1,'67cfa4b1-3ea3-4092-9716-4cb0e1341b77', NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);\n")
                        
                except Exception as e:
                    with open (r'D:\Cebolinha\OneDrive - Arquivar Franchising Ltda\General\TASK\Felipe\PYTHON\ASSOCIAR IMAGENS A REGISTROS ARQGED SEAC\\LogErro.txt', 'a') as arquivo:
                        arquivo.write(f"Erro: {e} | Blob: {source_blob_name}\n")
